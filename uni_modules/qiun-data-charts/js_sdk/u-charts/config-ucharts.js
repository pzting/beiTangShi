...
const cfu = {
	...
	"formatter": {
		...
		"tooltipRatio": function(item, category, index, opts) {
			/* console.log(item);
			console.log(category);
			console.log(index);
			console.log(opts);
			console.log('----'); */
			if (category && category.indexOf('率') > 0) {
				return `${item.name}:${item.data}%`
			} else {
				return `${item.name}:${item.data}`
			}
			/* else if (category && category.indexOf('收入') > 0) {
				return `${item.name}:${uni.$u.priceFormat(item.data,2)}`
			} else {
				return `${item.name}:${uni.$u.priceFormat(item.data)}`
			} */
